    ...
    const fetchData = async () => {
      try {
        ...
        if (userCardsData && userCardsData.length > 0) {
          ...
          const normalizedCards: CreditCard[] = userCardsData.map((card: UserCreditCardData) => {
        ...
        setError('message' in error ? error.message : 'Failed to load data. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
