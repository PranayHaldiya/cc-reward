...
export default function CardsPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userCardsData = await getUserCreditCards() as UserCreditCard[];
        setUserCards(userCardsData || []);

        // Fetch all available credit cards
        const allCardsData = await getAllCreditCards() as AvailableCreditCard[];
        
        // Filter out cards that the user already has
        const userCardIds = userCardsData ? userCardsData.map((card: UserCreditCard) => card.id) : [];
        const filteredAvailableCards = allCardsData ? allCardsData.filter((card: AvailableCreditCard) => !userCardIds.includes(card.id)) : [];
  ...
  const handleSubmitCard = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh user cards
      const updatedCards = await getUserCreditCards() as UserCreditCard[];
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const errorMessage = 'message' in error ? error.message : 'Failed to delete credit card. Please try again later.';
