...
export async function checkUserHasCard(cardId: string) {
  ...
  try {
    ...
    const foundCard = userCards.find((card: UserCard) => 
      card.id === cardId || 
    ...
    console.error('Error checking if user has card:', error instanceof Error ? error.message : 'Unknown error');
    return null;
  }
}
...
export async function deleteUserCreditCard(cardId: string) {
  ...
  try {
    // First, try to decode the JWT token to get the user ID
    const payload = JSON.parse(atob(token.split('.')[1]));
    const userIdFromToken = payload.sub; // JWT standard for subject (user ID)

    if (!userIdFromToken) {
      throw new Error('User ID not found in token');
    }

    const endpoint = `/api/users/${userIdFromToken}/credit-cards/${cardId}`;
